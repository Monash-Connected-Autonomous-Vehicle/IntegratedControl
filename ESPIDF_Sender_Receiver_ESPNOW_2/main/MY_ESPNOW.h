...
typedef struct ESDACanPayload {
    ESDACanMessageID messageID;
    float value;
} ESDA_Payload_Struct_t;
...
typedef struct {
    ...
    ESDA_Payload_Struct_t payload[0];
} __attribute__((packed)) espnow_data_t;    // gcc specific atribute (__attribute__((packed))) that prevents compiler from adding padding bytes between members of a structure
